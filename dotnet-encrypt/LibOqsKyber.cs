...
/// <summary>
/// Provides static methods for ML-KEM-1024 post-quantum key encapsulation operations
/// via native interop with the liboqs C library (through oqs_shim).
/// This class enables .NET applications to generate ML-KEM keypairs, encapsulate shared secrets,
...
public static class LibOqsKyber
{
    ...
    // Native P/Invoke signatures for ML-KEM-1024 operations
    [DllImport(LibName, CallingConvention = CallingConvention.Cdecl, ExactSpelling = true)]
    private static extern int oqs_ml_kem_1024_keypair(byte[] pk, UIntPtr pk_len, byte[] sk, UIntPtr sk_len);

    [DllImport(LibName, CallingConvention = CallingConvention.Cdecl, ExactSpelling = true)]
    private static extern int oqs_ml_kem_1024_encaps(byte[] pk, UIntPtr pk_len, byte[] ct, UIntPtr ct_len, byte[] ss, UIntPtr ss_len);

    [DllImport(LibName, CallingConvention = CallingConvention.Cdecl, ExactSpelling = true)]
    private static extern int oqs_ml_kem_1024_decaps(byte[] ct, UIntPtr ct_len, byte[] sk, UIntPtr sk_len, byte[] ss, UIntPtr ss_len);

    /// <summary>
    /// Generates a ML-KEM-1024 keypair (public and private keys) using the native liboqs library.
    /// </summary>
    ...
    public static (byte[] PublicKey, byte[] PrivateKey) GenerateKeyPair()
    {
        ...
        int rc = oqs_ml_kem_1024_keypair(pk, (UIntPtr)pk.Length, sk, (UIntPtr)sk.Length);
        if (rc != 0)
    ...
    /// <summary>
    /// Encapsulates a shared secret using the recipient's ML-KEM-1024 public key.
    /// Produces a ciphertext and a 32-byte shared secret suitable for AES-256.
    /// </summary>
    /// <param name="publicKey">The recipient's ML-KEM-1024 public key (1568 bytes)</param>
    ...
    public static (byte[] Ciphertext, byte[] SharedSecret) Encapsulate(byte[] publicKey)
    {
        ...
        int rc = oqs_ml_kem_1024_encaps(publicKey, (UIntPtr)publicKey.Length, ct, (UIntPtr)ct.Length, ss, (UIntPtr)ss.Length);
        if (rc != 0)
    ...
    /// <summary>
    /// Decapsulates a shared secret from a ML-KEM-1024 ciphertext using the private key.
    /// </summary>
    /// <param name="privateKey">The recipient's ML-KEM-1024 private key (3168 bytes)</param>
    /// <param name="ciphertext">The ML-KEM-1024 ciphertext (1568 bytes)</param>
    ...
    public static byte[] Decapsulate(byte[] privateKey, byte[] ciphertext)
    {
        ...
        var ss = new byte[SharedSecretLength];
        int rc = oqs_ml_kem_1024_decaps(ciphertext, (UIntPtr)ciphertext.Length, privateKey, (UIntPtr)privateKey.Length, ss, (UIntPtr)ss.Length);
        if (rc != 0)
