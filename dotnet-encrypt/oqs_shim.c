...
// Legacy Kyber1024 functions (for backward compatibility with older liboqs versions)
// These use OQS_KEM_alg_kyber_1024 algorithm identifier
...
// NIST-standardized ML-KEM-1024 functions (for liboqs 0.10.0+)
// These use OQS_KEM_alg_ml_kem_1024 algorithm identifier

// Returns 0 on success, non-zero on error
EXPORT int oqs_ml_kem_1024_keypair(uint8_t *pk, size_t pk_len, uint8_t *sk, size_t sk_len) {
...
// Returns 0 on success, non-zero on error
EXPORT int oqs_ml_kem_1024_encaps(const uint8_t *pk, size_t pk_len, uint8_t *ct, size_t ct_len, uint8_t *ss, size_t ss_len) {
    OQS_KEM *kem = OQS_KEM_new(OQS_KEM_alg_ml_kem_1024);
    if (kem == NULL) return 1;
    if (pk_len < kem->length_public_key || ct_len < kem->length_ciphertext || ss_len < kem->length_shared_secret) {
        OQS_KEM_free(kem);
...
// Returns 0 on success, non-zero on error
EXPORT int oqs_ml_kem_1024_decaps(const uint8_t *ct, size_t ct_len, const uint8_t *sk, size_t sk_len, uint8_t *ss, size_t ss_len) {
    OQS_KEM *kem = OQS_KEM_new(OQS_KEM_alg_ml_kem_1024);
    if (kem == NULL) return 1;
    if (ct_len < kem->length_ciphertext || sk_len < kem->length_secret_key || ss_len < kem->length_shared_secret) {
        OQS_KEM_free(kem);
